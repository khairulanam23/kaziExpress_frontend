...
const sheetVariants = cva(
  "bg-card fixed z-50 flex flex-col gap-4 border-border shadow-2xl transition ease-in-out data-[state=closed]:duration-200 data-[state=open]:duration-300",
  {
    variants: {
      side: {
        right:
...
function SheetContent({
  ...
  return (
    <SheetPortal>
      ...
      >
        {children}
        <DialogPrimitive.Close className="absolute top-4 right-4 rounded-md opacity-60 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-ring focus:outline-hidden cursor-pointer">
